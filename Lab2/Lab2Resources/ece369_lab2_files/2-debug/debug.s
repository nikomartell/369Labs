...
#	$t0 will hold the character that is being compared to the null character from str
#	$t1 holds the index of str 
#	$t2 holds the value of the loop count
#	$t3 holds 'char' which is used to compare against the current character stored in $t0
#	
# 2. Currently, the program is stuck in an infinite loop. Make use of 
#    breakpoints to locate, and correct the error.
#	The count of $t2 was not being incremented due to the jump to 'con' being executed before it. This led to an infinite loop on 'b' in 'str'.
